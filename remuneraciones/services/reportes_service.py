...
class ReportesService:
    ...
    def generar_resumen_ejecutivo(periodo, rut_empresa, razon_social):
        ...
        with open(nombre_archivo, 'w', newline='', encoding='utf-8-sig') as csvfile:
            ...
            writer.writerow(['Total Haberes', f"${stats['total_haberes']:,.0f}" if stats['total_haberes'] else '$0'])
            writer.writerow(['Total Descuentos', f"${stats['total_descuentos']:,.0f}" if stats['total_descuentos'] else '$0'])
            writer.writerow(['Total Líquido a Pagar', f"${stats['total_liquido']:,.0f}" if stats['total_liquido'] else '$0'])
            writer.writerow(['Promedio Líquido por Empleado', f"${stats['promedio_liquido']:,.0f}" if stats['promedio_liquido'] else '$0'])
