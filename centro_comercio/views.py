...
from .forms import ClienteForm, CotizacionForm, CotizacionItemFormSet, CotizacionItemFormSetEdit, CotizacionItemForm, CotizacionCuotaFormSet
from .models import Cliente, Cotizacion, CotizacionEstado, Vendedor, CotizacionItem, CotizacionCuota
...
def clientes_lista(request):
    ...
    return render(request, "centro_comercio/clientes/_tabla.html", {"clientes": qs, "q": q})
...
def cliente_form(request, pk: int = None):
    ...
    if request.method == "POST":
        ...
        return render(request, "centro_comercio/clientes/_form.html", {"form": form, "obj": obj}, status=400)
...
def vendedores_lista_api(request):
    ...
    data = [
        {"id": v.id, "text": f"{v.nombre} - {v.comision_porcentaje}%", "comision": str(v.comision_porcentaje)}
        for v in qs[:20]
    ]
    return JsonResponse({"results": data})

# ============================================================
# COTIZACIONES - VERSIÓN DEFINITIVA
# ============================================================
@login_required
...
def cotizaciones_lista(request):
    ...
    qs = qs.order_by("-id")
    return render(request, "centro_comercio/cotizaciones/_tabla.html", {
        "cotizaciones": qs, "q": q, "estado": estado,
        "estados": CotizacionEstado.choices, "fecha": fecha_str,
    })

@login_required
def cotizacion_detalle(request, pk: int):
    obj = get_object_or_404(Cotizacion.objects.select_related("cliente", "vendedor").prefetch_related("items", "cuotas"), pk=pk)
...
def cotizacion_form(request, pk: int = None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid() and cuota_formset.is_valid():
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    print("Código será generado automáticamente por el modelo")

                    cot.save()

                    print(
                        f"Cotización guardada: "
                        f"ID={cot.id}, "
                        f"Código={cot.codigo}, "
                        f"Número={cot.numero}"
                    )
                ...
                print(f"ERROR en cotizacion_form: {str(e)}")
        ...
        print("Formset items errors:", formset.errors)
        print("Formset cuotas errors:", cuota_formset.errors)
    ...
    if obj is None:
        ...
        form = CotizacionForm(
            initial={
                ...
                "codigo": "",
                "condicion_venta": "CRED"  # Valor por defecto: Crédito
            },
            user=request.user
        )
...
def cotizacion_cliente_info(request, cliente_id: int):
    ...
    data = {
        "id": cliente.id, "rut": cliente.rut, "razon_social": cliente.razon_social,
        "giro": cliente.giro, "telefono": cliente.telefono, "email": cliente.email,
...
def cotizacion_resumen_api(request, pk: int):
    obj = get_object_or_404(Cotizacion.objects.select_related("cliente", "vendedor").prefetch_related("items"), pk=pk)
    items_data = []
    for it in obj.items.all():
        items_data.append({
            "id": it.id, "titulo": it.titulo, "unidad": it.unidad,
            "cantidad": str(it.cantidad), "valor_unitario": str(it.valor_unitario),
            "exento": it.exento, "descuento_porcentaje": str(it.descuento_porcentaje),
            "descuento": str(it.descuento), "total": str(it.total),
        })
    data = {
        "id": obj.id, "codigo": obj.codigo,
        "cliente": {"id": obj.cliente.id, "razon_social": obj.cliente.razon_social, "rut": obj.cliente.rut},
        "fecha": obj.fecha.isoformat(), "vigencia_hasta": obj.vigencia_hasta.isoformat(),
        "sucursal": obj.sucursal, "vendedor": {"id": obj.vendedor.id, "nombre": obj.vendedor.nombre} if obj.vendedor else None,
        "comision_porcentaje": str(obj.comision_porcentaje), "descuento_porcentaje": str(obj.descuento_porcentaje),
        "descuento": str(obj.descuento), "recargo_porcentaje": str(obj.recargo_porcentaje),
        "glosa": obj.glosa, "observaciones": obj.observaciones, "terminos": obj.terminos,
        "estado": obj.estado, "estado_display": obj.get_estado_display(),
        "totales": {"subtotal": str(obj.subtotal), "total_afecto": str(obj.total_afecto),
                    "total_exento": str(obj.total_exento), "iva": str(obj.iva),
                    "descuento_aplicado": str(obj.descuento_aplicado),
                    "recargo_aplicado": str(obj.recargo_aplicado), "total_neto": str(obj.total_neto)},
        "items": items_data,
    }
    return JsonResponse(data)
...
def cotizacion_duplicar(request, pk: int):
    ...
    return redirect(f"{reverse('centro_comercio:cotizaciones_panel')}?open={nueva.id}")
...
# ============================================================
# PDF - GENERAR DOCUMENTO (VERSIÓN ÚNICA Y CORREGIDA)
# ============================================================
@login_required
def cotizacion_pdf(request, pk: int):
    """Generar PDF de la cotización - Versión corregida sin duplicados"""
    from django.http import HttpResponse
    from django.template.loader import get_template
    
    obj = get_object_or_404(
        Cotizacion.objects.select_related("cliente", "vendedor")
        .prefetch_related("items", "cuotas"), 
        pk=pk
    )
    
    # Renderizar template HTML (se abrirá en nueva pestaña y podrá imprimirse)
    return render(
        request, 
